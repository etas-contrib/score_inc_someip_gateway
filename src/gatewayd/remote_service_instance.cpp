        ...
        client_config, inst,
        {
            .on_service_state_change =
                [this](socom::Client_connector const&, socom::Service_state state,
                    ...
                    for (std::size_t i = 0; i < service_type_config_->events()->size(); ++i) {
                        (void)client_connector_->subscribe_event(
            ...
            .on_event_update =
                [this](socom::Client_connector const&, socom::Event_id event_id,
                    ...
                    forward_event(event_id, std::move(payload));
                },
            ...
            .on_event_payload_allocate =
                [](socom::Client_connector const&,
                   socom::Event_id) -> score::Result<socom::Writable_payload> {
                    ...
                    return socom::Writable_payload{
                        span, socom::kNoSlotHandle,
...
void RemoteServiceInstance::forward_event(socom::Event_id event_id, socom::Payload payload) {
    ...
    std::memcpy(sample.Get(), src.data(), std::min(src.size(), ipc_event.GetSizeInfo().size));

    ipc_event.Send(std::move(sample));
}

void RemoteServiceInstance::CreateAsyncRemoteService(
    std::shared_ptr<const mw_someip_config::ServiceInstance> service_instance_config,
    std::shared_ptr<const mw_someip_config::ServiceType> service_type_config,
    socom::Runtime& socom_runtime,
    ...
    for (const auto& event : *service_type_config->events()) {
        ...
        score::mw::com::DataTypeMetaInfo const type_info{someip::kMaxMessageSize,
                                                         alignof(std::byte)};
        events.emplace_back(
