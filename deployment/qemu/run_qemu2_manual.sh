#!/bin/bash
# Manual debug launcher for QEMU Instance 2 (192.168.87.3)
#
# Usage:
...
#        ./deployment/qemu/run_qemu2_manual.sh --daemons
...
if [[ "${START_SERVICES}" == "true" ]]; then
    ...
    deployment/qemu/run_qemu.sh "${IFS_IMAGE}" 2 &
    QEMU_PID=$!
    trap "kill ${QEMU_PID} 2>/dev/null" EXIT

    echo "Waiting for QEMU 2 to boot..."
    ...
    echo "Starting sample_client..."
    ssh ${SSH_OPTS} -f ${SSH_USER}@${GUEST_IP} \
        "export VSOMEIP_CONFIGURATION=/etc/sample_client/vsomeip.json; /usr/bin/sample_client > /dev/null 2>&1"
    sleep 2

    echo "Services started on QEMU 2 (${GUEST_IP}). Dropping into SSH shell..."
    ...
    exec deployment/qemu/run_qemu.sh "${IFS_IMAGE}" 2
fi
