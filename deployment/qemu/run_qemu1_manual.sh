...
#        bazel build //deployment/qemu:init_a --config=x86_64-qnx
#   2. Run QEMU only:
...
IFS_IMAGE="bazel-bin/deployment/qemu/someip_gateway_x86_64_a.ifs"
...
if [[ ! -f "${IFS_IMAGE}" ]]; then
    ...
    echo "Build it first: bazel build //deployment/qemu:init_a --config=x86_64-qnx"
    exit 1
fi
